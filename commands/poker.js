      ...
      const embed = new EmbedBuilder()
        ...
        .addFields(
          ...
          },
          {
            name: "🤖 HOUSE HAND",
            value: `> ${
              resultData ? houseHand.map((c) => c.id).join(" ") : "❓ ❓"
            }`,
            inline: true,
          },
          {
...
\`\`\``,
    ...
    collector.on("collect", async (i) => {
      ...
      if (!activePoker.has(user.id)) return;
      ...
      const freshUser = await User.findOne({ userId: user.id });
      if (freshUser.gold < callAmount)
        return i.reply({
      ...
      let dealerQualifies = false;

      if (bRes.label === "Pair") {
        const counts = {};
        [...houseHand, ...community].forEach(
          (c) => (counts[c.val] = (counts[c.val] || 0) + 1),
        );

        const pairRanks = Object.entries(counts)
          .filter(([_, count]) => count >= 2)
          .map(([val]) => order.indexOf(val))
          .sort((a, b) => b - a);

        if (pairRanks.length > 0) dealerQualifies = pairRanks[0] >= 2;
      } else if (bRes.label !== "High Card") {
      ...
      if (!dealerQualifies) {
        payout = Math.floor(currentBet * 1.6) + callAmount;
      } else if (playerWins) {
        payout = Math.floor(currentBet * 1.6) + Math.floor(callAmount * 1.6);
      } else if (isPush) {
      ...
      const totalInvested = currentBet + callAmount;

      const resultData = {
        payout,
        net: payout - totalInvested,
        balance: finalUser.gold,
      };

      const finalStatus = !dealerQualifies
        ? `⚠️ NO QUALIFY (${pRes.label})`
        : playerWins
          ? `✅ WIN! (${pRes.label})`
          : isPush
            ? `🤝 PUSH`
      ...
      logToAudit(client, {
        ...
        oldBalance: userData.gold - currentBet - callAmount,
        newBalance: finalUser.gold,
        reason: `Poker: ${pRes.label} vs ${bRes.label} (Qualify: ${dealerQualifies})`,
      });
    });
