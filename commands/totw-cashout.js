...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const VALID_CHANNEL_ID = "1435117294454833212";
    ...
    if (interaction.channelId !== VALID_CHANNEL_ID) {
      return interaction.reply({
        content: `❌ **Wrong Channel:** This command can only be used in <#${VALID_CHANNEL_ID}>.`,
    ...
    // 3. SECURE ENV CHECK (Prevents crashes if .env is missing data)
    if (!DEPT_GAME_ID || !DEPT_GAME_PASS) {
      return interaction.reply({
        content:
          "🚨 **System Error:** TOTW Account credentials are not configured in the bot's environment.",
        ephemeral: true,
      });
    }
