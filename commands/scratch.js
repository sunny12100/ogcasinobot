...
// Global cleanup (prevents permanent locks)
setInterval(() => {
  const now = Date.now();
    ...
    try {
      ...
      if (activeScratch.has(user.id)) {
        return interaction.editReply(
          "❌ You already have an card active! Finish it first.",
        );
      }
        ...
        try {
          ...
          for (let ri = 0; ri < 5; ri++) {
            for (let ci = 0; ci < 5; ci++) {
              if (ri !== r || ci !== c)
                allOtherPositions.push({ r: ri, c: ci });
            }
          }

          shuffle(allOtherPositions);

          const allWinners = [];

          if (mult < 10 && allOtherPositions.length) {
            const p = allOtherPositions.pop();
            allWinners.push({ pos: `scr_${p.r}_${p.c}`, icon: "🏆" });
          }

          for (let j = 0; j < 9 && allOtherPositions.length; j++) {
            const p = allOtherPositions.pop();
            allWinners.push({ pos: `scr_${p.r}_${p.c}`, icon: "🎫" });
          }
      ...
      console.error("Fatal Scratch Command Error:", fatalErr);
